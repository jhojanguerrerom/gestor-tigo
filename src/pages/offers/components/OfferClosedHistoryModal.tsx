...
export default function OfferClosedHistoryModal({ isOpen, onClose, ofertaId }: OfferClosedHistoryModalProps) {
  ...
  // --- HELPERS ---
  // 🟢 Traduce operaciones y acciones de base de datos o estado al español
  ...
  // Formatea de manera segura los valores antiguos y nuevos de los campos
    ...
    try {
      ...
      if (changesResult.status === 'fulfilled') {
        ...
        setChangesTotalPages(
          Math.ceil(
            (changesResult.value.data?.total_cambios || 0) / CHANGES_LIMIT
          )
        );
      } else {
                      ...
                      {changes.map((item) => (
                        <div key={item.id} className="mb-4 position-relative py-1">
                          ...
                          <div className="d-flex justify-content-between align-items-center mb-2">
                            {/* 🟢 Cambiado: Aplica formatAction para traducir el tipo de operación */}
                  ...
                  <div className="position-relative ps-3 border-start border-start-4 border-primary ms-1">
                    {history.map((item) => (
                      <div key={item.id} className="mb-4 position-relative py-1">
                        <div className="position-absolute bg-primary rounded-circle border border-white border-3" style={{ width: '18px', height: '18px', left: '-1.6rem', top: '0.45rem' }}></div>
                        <div className="d-flex justify-content-between align-items-center mb-2">
                          {/* 🟢 Cambiado: Traduce la acción de la gestión del estado */}
                          <span className="badge bg-primary p-1 fw-bold text-uppercase">
                            {item.accion}
                          </span>
                  ...
                  <div className="position-relative ps-3 border-start border-start-4 border-primary ms-1">
                    {details.map((item) => (
                      <div key={item.id} className="mb-4 position-relative py-1">
                        ...
                        <div className="d-flex justify-content-between align-items-center mb-2">
                          <div>
                            {/* 🟢 Cambiado: Traduce los badges de acción y subacción en las observaciones */}
